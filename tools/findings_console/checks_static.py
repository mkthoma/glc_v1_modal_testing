...
CHECKS: list[Check] = [
    Check(
        ...
        plan_task="T1.13",
        attacker_role="AR3",
        command='grep -nE "modal\\.Sandbox|outbound_domain_allowlist|block_network" modal_app.py',
        fix_summary=(
            "modal_app.py's verify_telegram_egress_allowlist() local_entrypoint uses modal.Sandbox.create"
            "(..., outbound_domain_allowlist=TELEGRAM_EGRESS_ALLOWLIST) for telegram. Container "
            "separation (Move B/C) is done for all 15 adapters; the egress allowlist itself is only "
            "demonstrated for telegram, not wired into every adapter's live webhook dispatch path — "
    ...
    Check(
        ...
        attacker_role="AR3",
        command="grep -n 'ADAPTER_SECRETS' -A 20 modal_app.py",
        fix_summary=(
            "modal_app.py's ADAPTER_SECRETS dict maps every one of the 15 catalogue adapters to its own "
            "Secret name (or None for local_mic/webui, which need no external credential) — never "
            "glc-llm-keys, the Secret the core gateway mounts for provider keys. This check parses that "
            "dict directly and reports CLOSED only if every non-null entry is distinct and none collides "
            "with glc-llm-keys."
        ),
    ),
    Check(
        ...
        attacker_role="AR4",
        command='grep -nE "uv_sync|from_registry|@sha256:" modal_app.py',
        fix_summary=(
            "modal_app.py builds from modal.Image.from_registry('python:3.11-slim@sha256:...') (a "
            "digest-pinned base) piped into .uv_sync(extra_options='--no-dev'), which runs `uv sync "
    ...
    Check(
        ...
        attacker_role="AR1",
        command="grep -n 'max_containers' modal_app.py",
        fix_summary=(
            "modal_app.py pins max_containers=1 on the core gateway Function. Mitigated, not closed: "
            "this trades away horizontal scalability to guarantee a single writer rather than adding "
            "real coordination — a dedicated writer process or a managed DB would close it properly."
        ),
    ),
    Check(
        id="L7",
        ...
        plan_task="T1.16",
        attacker_role="AR3",
        command="grep -n 'shutil.which\\|WHISPER_CLI_PATH' glc/voice/stt/providers/whisper_cpp/wrapper.py",
        fix_summary=(
            "glc/voice/stt/providers/whisper_cpp/wrapper.py resolves WHISPER_CLI_PATH from "
...
def _check_a4() -> CheckResult:
    src = _read(MODAL_APP)
    if src.startswith("__READ_ERROR__"):
        return CheckResult("A4", "local checkout", CheckKind.STATIC, Verdict.ERROR, src, src)
