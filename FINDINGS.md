...
**Status:** a genuinely separated, un-monkey-patchable *path* now exists (`evaluate_remote()`), but the *original* vulnerable function (`glc.policy.engine.evaluate`) is completely unmodified and exactly as exploitable as before — see the correction below before trusting "closed"

**Second correction to this file's L5 entry** (the first correction, replaced below, overclaimed): I built `glc-policy-engine` as a separate Modal Function and verified that calling it via `.remote()` is unaffected by a local monkey-patch — that part is true and real. But I then wrote "mechanism fully closed for every attacker tier," which overstates it. **The exploit exactly as the finding shows it — `import glc.policy.engine as e; e.evaluate = lambda *a, **k: PolicyVerdict(action="allow", reason="pwned")` — still succeeds completely, unconditionally, deployed or not**, because `glc/policy/engine.py` itself was never touched. Monkey-patching replaces the *entire function object*; there is no way to make a directly-importable, directly-callable function resist being replaced by whoever holds a reference to its module — the only thing that actually stops the exploit is a caller that *never holds a reference to the local function in the first place* and calls the separated Function instead. Found by checking exactly what the `findings_console`'s L5 check tests (it monkey-patches `glc.policy.engine.evaluate` and calls that same function again) and confirming its `vulnerable` verdict is correct, not a tooling limitation like L1/L3/L4/L8's `vulnerable` results are.
**What's actually true:** a new, genuinely separated path (`glc-policy-engine`, a Modal Function with no Secret and no Volume mount, rules from the packaged `policy.yaml`) exists and is provably un-monkey-patchable from any calling process, verified live — a local tamper of `glc.policy.engine.evaluate` had zero effect on a call to the real deployed Function. `GLC_POLICY_ENGINE_REMOTE=1` is set on the core gateway's env and `glc/policy/remote.py`'s `evaluate_remote()` is ready to use it.
**What's NOT true:** that this closes the finding as stated. Nothing in the codebase calls `evaluate_remote()` today — nothing calls `glc.policy.engine.evaluate` either (the agent runtime is still a stub; both are only referenced from test files) — so there is currently no live call site for either the exploit or the fix to act on. If and when policy enforcement is wired into a real decision path, this is only closed if that new code calls `evaluate_remote()` specifically. If it calls the local `evaluate()` instead — which is exactly as easy to write, and the more obvious thing to reach for — the finding is exactly as open as the day it was written, with a safer alternative sitting unused next to it.
**Honest overall status:** open, in the sense the original finding is written and the console correctly tests. A working, verified alternative exists and is deployed, but does not by itself close anything until something is actually changed to use it instead of the vulnerable function it doesn't replace or remove.
...
- **L5 remains open — the original vulnerable function was never modified, and nothing calls its replacement.** `glc-policy-engine` (a separate Modal Function) and `glc/policy/remote.py`'s `evaluate_remote()` are real, deployed, and verified live to be unaffected by a local monkey-patch of `glc.policy.engine.evaluate`. But `glc/policy/engine.py` itself is completely untouched — the exact exploit the finding names (`e.evaluate = lambda *a, **k: PolicyVerdict(action="allow", ...)`) still succeeds against it, unconditionally, whether or not the gateway is deployed. Confirmed directly: the `findings_console`'s L5 check monkey-patches that exact function and calls it again in the same process, and correctly reports `vulnerable` — this is not a tooling limitation the way L1/L3/L4/L8's `vulnerable` results are; it's an accurate result. Nothing currently calls either the vulnerable function or its replacement for a real decision (the agent runtime is still a stub), so there's no live exploit today — but if and when policy enforcement is wired in, it closes only if that new code calls `evaluate_remote()` specifically. Calling the local `evaluate()` — the more obvious thing to reach for — leaves this exactly as open as before, with a safer, unused alternative sitting next to it.
