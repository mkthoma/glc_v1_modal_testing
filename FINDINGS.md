...
This is the required note: for each finding, the invariant it broke and the fix that closed or mitigated it. Findings are grouped the same way the local testing dashboard (`tools/findings_console/`) groups them — by what the Modal migration did to each one. Every fix below has its own commit; each entry's `Commit:`/`Fix commit(s):` line gives the short hash directly (`git show <hash>` in this repo's own history) alongside the subject for context.

The repo now ships two deployable variants so every "before" value quoted below is a real, live measurement rather than a description of what used to be true: `with_fixes/` is the hardened gateway all fixes below apply to, and `without_fixes/` is a frozen, byte-identical snapshot of the code exactly as it stood before any of this hardening, deployed as its own separate Modal app purely to give the findings console something real to test against on the "before" side. All `with_fixes/glc/...` and `with_fixes/modal_app.py` paths below are correspondingly relative to that split — most of the commits quoted below predate the move (`7ffeb1b`) into `with_fixes/`, so `git show` on one of the earlier hashes shows the file at its original bare `glc/...` path.

Two things worth being upfront about, since they shape how "closed" is used below:

- Only **C2/L9** (cross-channel envelope spoofing) is closable with a pure application-layer code change. **L1–L5, L7, L8, L10 fundamentally require process/container separation to be fully closed** — Python has no in-process ACL on `os.environ` or on importable functions, so the only real wall is a kernel-enforced process boundary. Once Move B/C put every catalogue adapter in its own real container, that boundary actually exists now, and it closes more of these than earlier drafts of this file gave it credit for: **L1, L3, L4, L8 are fully closed for AR3** (an attacker who has compromised a single adapter container — verified live, not assumed). All four are verified this way *permanently and repeatably*, not just once by hand: `with_fixes/modal_app.py` deploys `glc-adapter-shape-probe` (env/pairing/token checks) and `glc-adapter-shape-self-kill-probe` (a separate Function so a self-kill test never risks the read-only ones), both built with the exact adapter-image shape, and the `findings_console`'s L1/L3/L4/L8 checks call them directly and report a real, live-measured verdict every time you click Run — no more "always shows vulnerable, trust the docs instead" footnote for any of them. (An earlier draft kept L8 local-only after a self-kill test hung the Modal CLI once — a follow-up experiment found calling `.remote()` instead of `modal run` raises a clean exception in ~20-30s, so it converted too.) **L2, L7, L10 got a real Part-1-scope mitigation** (hash-chaining, an absolute binary path, input validation). **L5 got a real, direct code fix this round** — `with_fixes/glc/policy/engine.py` itself now blocks the exact monkey-patch the finding names (`__slots__` + a frozen module class) — but it's **mitigated, not closed**: an equally-easy `__dict__`-write bypass, one line long, still fully succeeds for the same AR4 attacker, and the separated `glc-policy-engine` path from an earlier draft remains real but still uncalled. Read L5's own entry before trusting any summary of it, including this one — two earlier drafts of this bullet each overclaimed differently (first "no mitigation possible," then "fully closed for both AR3 and AR4"), for two different reasons. L1/L3/L4/L8 remain open for AR4 specifically (code execution inside the gateway process itself, which still holds the real Volume-backed data those four protect) — closing that needs a further split of the gateway's own trusted internals from its request-handling code, a materially bigger, different-shaped task than "every adapter gets a container," out of scope here. Check each entry's own Status line rather than assuming from this summary.
...
**Fix:** `with_fixes/glc/security/auth.py`'s `require_data_plane_token` (constant-time `hmac.compare_digest` from the start), wired via `dependencies=[Depends(...)]` on all six routes. Commit: `dce704d` — harden: require bearer-token auth on the data plane (A1).
...
**Fix:** same `require_data_plane_token` dependency applied to all seven endpoints; `main.py` sets `docs_url`/`redoc_url`/`openapi_url` to `None` when `GLC_ENV=production` (set in the Modal image). Commits: `4428d7e` — harden: gate info-disclosure endpoints, disable docs in prod (A2); `8ad9b94` — harden: set GLC_ENV=production in the Modal image (A2 follow-up).
...
**Correction from an earlier draft of this file:** the first pass at Move B/C migrated only telegram, on the belief (inherited from the assignment's own framing, never independently checked) that the other 14 catalogue adapters were still `NotImplementedError` stubs not worth containerizing yet. That was wrong — `grep -rn "raise NotImplementedError" with_fixes/glc/channels/catalogue/*/adapter.py` returns nothing; every adapter has a real implementation. There was no remaining reason to leave 14 of them sharing the core gateway's LLM Secret, so all 15 are now migrated.
**Fix:** every catalogue adapter now runs in its own Modal Function via `with_fixes/modal_app.py`'s `ADAPTER_SECRETS` mapping + `make_adapter_functions()`; the core gateway Function never imports adapter code in-process. Egress allowlisting via `modal.Sandbox` remains demonstrated only for telegram (Move D). Commits: `b2d8306` — harden: per-adapter container + scoped Secret, telegram (Move B/C); `615605b` — harden: migrate all 15 catalogue adapters to per-adapter Functions + Secrets (Move B/C gateway-wide); `9ac4177` — harden: egress allowlist via Modal Sandbox, telegram (Move D).
**Re-verified:** `uv run modal run with_fixes/modal_app.py::verify_telegram_egress_allowlist` — a request to `api.telegram.org` succeeds, a request to `example.com` is blocked. The egress wall itself is **not** wired into the live per-request webhook dispatch path for any adapter (see Move D note below), and the core gateway Function still has open egress, which it needs to reach LLM providers — that part of A3 stays "mitigated," not closed.
...
**Fix:** `with_fixes/modal_app.py`'s `ADAPTER_SECRETS` dict maps every one of the 15 catalogue adapters to its own Secret (`glc-discord-secret`, `glc-gmail-secret`, `glc-imap-secret`, `glc-line-secret`, `glc-matrix-secret`, `glc-signal-secret`, `glc-slack-secret`, `glc-teams-secret`, `glc-telegram-secret`, `glc-twilio-sms-secret`, `glc-twilio-voice-secret`, `glc-webhook-secret`, `glc-whatsapp-secret`, or `None` for `local_mic`/`webui`, which need no external credential) — never `glc-llm-keys`. Commits: `b2d8306` — harden: per-adapter container + scoped Secret, telegram (Move B/C); `615605b` — harden: migrate all 15 catalogue adapters to per-adapter Functions + Secrets (Move B/C gateway-wide).
...
**Fix:** `modal.Image.from_registry("python:3.11-slim@sha256:e031123e...")` (digest resolved via the Docker Hub registry API) piped into `.uv_sync(extra_options="--no-dev")`, which runs `uv sync --frozen` against this repo's own lock file. Commit: `5db94f4` — harden: reproducible image build from uv.lock, pin base digest (A5).
...
Same fix and verification as A4 above — this is A4's code-level consequence, closed in the same commits (`b2d8306`, `615605b`), plus `bf846cd` — feat: live-verify L1/L3/L4 against the deployed adapter shape, not a local assumption. **The `findings_console`'s L1 check now verifies this live**, not from a local assumption: it calls `glc-adapter-shape-probe`, a Modal Function deployed with the exact same image shape as a real catalogue adapter (`adapter_image()`, no LLM Secret, no Volume mount — see `with_fixes/modal_app.py`), and reports whether `GEMINI_API_KEY` is actually present in that container's environment. Re-run any time with **Run** on L1 in the dashboard; it calls the real deployed system every time, not a documented assumption.
...
**Fix:** every row now carries `hash = sha256(prev_hash + canonical_json(row))`, chained off the previous row. `verify_chain()` walks the table and reports the first row where content or chain linkage no longer matches. Commit: `da02dfd` — harden: hash-chained audit log (L2/L3 defense-in-depth).
...
**Re-verified live, and now permanently, not just once:** the throwaway probe used to check this by hand is now `glc-adapter-shape-probe`, a Modal Function deployed alongside the gateway with the exact adapter-image shape (no Volume, no `GLC_CONFIG_DIR`/`GLC_PAIRING_DB`) — and the `findings_console`'s L3 check calls it directly. `os.path.isdir("/data")` from inside it returns `False`: there is no `/data` mount to write to at all, regardless of what code runs. `force_pair_owner()` still runs and creates a record — Python can't block the call itself — but only in that container's own throwaway, ephemeral filesystem, confirmed by the same probe call's `pairing_write_landed: True` alongside `data_mount_exists: False`. Re-run any time with **Run** on L3 in the dashboard.
...
**Re-verified live, permanently:** same `glc-adapter-shape-probe` Function as L3 — no `/data` mount exists in an adapter-shaped container at all, confirmed by the same call's `data_mount_exists: False`; `install_token_created: True` alongside that confirms `get_or_create_install_token()` only ever creates a throwaway local token there, never reaches or forges the real one. The `findings_console`'s L4 check calls this Function directly — re-run any time with **Run** on L4 in the dashboard.
...
**Location:** `with_fixes/glc/policy/engine.py`; `with_fixes/glc/policy/remote.py` (new)
...
**What's still true from the second correction:** the separated `glc-policy-engine` Modal Function (no Secret, no Volume mount, rules from the packaged `policy.yaml`) is real, deployed, and immune to *both* the blocked and the still-working local tamper technique, since it never references the local module at all. `GLC_POLICY_ENGINE_REMOTE=1` is set on the core gateway's env and `with_fixes/glc/policy/remote.py`'s `evaluate_remote()` is ready to use it. Nothing in the codebase calls it yet — the agent runtime is still a stub — so it remains the only mechanism that would fully close this for AR4, once something actually calls it instead of the local `evaluate()`.

### L6 — Unbounded egress

**Location:** `with_fixes/modal_app.py` (same root as A3)
...
**Fix:** `WHISPER_CLI_PATH` now resolves from `GLC_WHISPER_CLI_PATH` (default `/usr/local/bin/whisper-cli`), an absolute path checked with `.is_file()` before use — no `PATH` search at all. Commit: `f53cf44` — harden: whisper_cpp binary via configured path, not PATH (L7).
...
**Location:** `with_fixes/glc/routes/control.py:114` — `os.kill(os.getpid(), signal.SIGTERM)`
**Invariant broken:** INV-6
...
**Now verified live, permanently, not just structurally argued:** the first attempt at an empirical self-kill test hung the local Modal CLI client indefinitely (a scratch probe self-killing under `modal run` — likely internal retry/reconnect behavior on an abnormal container exit), so an earlier draft of this entry settled for the structural argument alone. Prompted again on whether that risk could actually be worked around, a small experiment showed calling `.remote()` (the SDK call the console already uses everywhere, not `modal run`'s ephemeral script mode) on a self-killing Function raises a clean, catchable exception after ~20-30s instead of hanging. `with_fixes/modal_app.py` now deploys `glc-adapter-shape-self-kill-probe` — a second Function, kept separate from `glc-adapter-shape-probe` so a self-kill test never risks the read-only checks — and the `findings_console`'s L8 check calls it directly: confirms the real gateway's `/healthz` is healthy, calls the self-kill probe (expects an exception — confirmed: `InternalFailure: Server has lost track of input`), then confirms `/healthz` immediately after. Re-run any time with **Run** on L8 in the dashboard (takes ~25-30s, not instant).
...
**Fix:** `log_call()` now rejects negative values and values above a 2,000,000-per-call ceiling (`MAX_TOKENS_PER_CALL`) with `ValueError`, before the `INSERT` runs. Commit: `3ea39be` — harden: validate cost-ledger writes (L10).
...
**Fix:** immediately after envelope validation, reject and close (`WS_1008_POLICY_VIOLATION`) any message where `env.channel != name`, checked on every message inside the `while True` loop, not just at connect time; the attempt is audit-logged as `channel_spoof_attempt`. Commits: `519db0b` — harden: reject cross-channel envelope spoofing over WS (C2/L9); `e1c21ce` — fix: C2/L9 skips re-pairing once already paired, immune to C6's lockout (console-side testability fix, not a security change).
...
**Fix:** removed the `token` query param and its fallback branch entirely; header-only bearer auth. The two dev bridge scripts that built `?token=` URLs were updated to use `additional_headers` instead. Commit: `80b8b29` — harden: WS channel auth via header only (C3).
...
**Fix:** every site logs the full detail server-side (`logger.error(..., exc_info=True)`) and raises a generic client-facing message instead — the image-fetch failure, both provider-call failure sites, and the embed error paths (429/400/502/503). Commit: `d1c4808` — harden: generic client errors, detailed server-side logs (C4).
...
**Fix:** `PairingStore` tracks confirm failures in a sliding window (10 per 5 minutes); once hit, `confirm_code` raises `PairingLockedOut` and the route returns `429`. This is a global lockout, not per-identity — a wrong guess doesn't reveal whose pairing it was meant for. Commit: `04ce55a` — harden: pairing-code attempt limiter (C6).
...
**Fix:** both call sites now use `hmac.compare_digest`. Commit: `d183a21` — harden: constant-time token comparisons (control.py, channels.py).
