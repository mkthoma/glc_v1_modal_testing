...
**Why this one stayed a local demonstration when L1/L3/L4 became live checks:** those three converted to calling `glc-adapter-shape-probe` live because reading an env var and attempting a file/DB write are safe, side-effect-limited operations to repeat on demand. Actually calling `os.kill()` on that same deployed Function risks the exact hang above, every time the check runs — not worth risking the console's own reliability for a fact already confirmed structurally.
