...
- Only **C2/L9** (cross-channel envelope spoofing) is closable with a pure application-layer code change. **L1–L5, L7, L8, L10 fundamentally require process/container separation to be fully closed** — Python has no in-process ACL on `os.environ` or on importable functions, so the only real wall is a kernel-enforced process boundary. Once Move B/C put every catalogue adapter in its own real container, that boundary actually exists now, and it closes more of these than earlier drafts of this file gave it credit for — twice: **L1, L3, L4, L8 are fully closed for AR3** (an attacker who has compromised a single adapter container — verified live, not assumed); **L5 is fully closed for both AR3 and AR4** (the policy decision now runs in its own Modal Function, `glc-policy-engine`, with no Secret and no Volume — verified live that monkey-patching the local `evaluate` reference has zero effect on the real decision, even from a process standing in for the gateway itself); **L2, L7, L10 got a real Part-1-scope mitigation** (hash-chaining, an absolute binary path, input validation). L5's fix is architecture that's deployed and functionally proven, but not yet exercised by a production decision path (nothing calls the policy engine for a real decision today — the agent runtime is still a stub). L1/L3/L4/L8 remain open for AR4 specifically (code execution inside the gateway process itself, which still holds the real Volume-backed data those four protect) — closing that needs a further split of the gateway's own trusted internals from its request-handling code, a materially bigger, different-shaped task than "every adapter gets a container," out of scope here. Check each entry's own Status line rather than assuming from this summary.
...
**Status:** mechanism fully closed for every attacker tier (AR3 and AR4), verified live — not yet exercised by a production decision path

**Correction from an earlier draft of this file:** I initially marked this "genuinely deferred, no mitigation possible," reasoning that self-checking code can't certify its own integrity against an attacker at the same privilege level (true) and concluding from that, incorrectly, that *nothing* could be done. That conflated two different things: an in-process integrity check *around* `evaluate()` (genuinely impossible — an attacker can patch the check too) with *moving the decision to a different process entirely* (exactly the Move B/C pattern already built for adapters, and just as applicable here).
**Fix:** `glc-policy-engine`, a new Modal Function with no Secret and no Volume mount — its only inputs are the `tool_call`/`context` dicts passed on each call, and its rules come from the packaged `policy.yaml` baked into its own image. `glc/policy/remote.py`'s `evaluate_remote()` calls it via `.remote()` instead of the local `glc.policy.engine.evaluate`. `GLC_POLICY_ENGINE_REMOTE=1` is set on the core gateway's env.
**Why this closes it for AR4 too, unlike L3/L4/L8:** those three still leave the *real data* (pairing store, install token) inside the gateway's own container, so an attacker with code execution there (AR4) still reaches it directly. Here, the *decision-making code itself* runs only inside the separate `glc-policy-engine` container — a monkey-patch in the calling process (gateway or adapter, AR3 or AR4) can only rebind that process's own local reference, which `evaluate_remote()` never calls.
**Re-verified live:** deployed the real `glc-policy-engine` Function, then in a separate local process monkey-patched `glc.policy.engine.evaluate` to always return `allow, pwned`, confirmed the local (tampered) call reflects that, then called the real deployed Function with a request the packaged `policy.yaml` denies (`shell.exec` with `sudo rm -rf /`, matching the shell deny-list rule) — the Function returned `{"action": "deny", "reason": "command pattern is on the shell deny list", ...}`, completely unaffected by the local tamper.
**Honest caveat:** nothing in the current route handlers calls the policy engine for a real decision yet — the agent runtime is still a stub that just echoes messages (`glc/routes/channels.py`), matching the state of `glc.policy.engine.evaluate` itself, which is also only referenced from test files today. This fix is real, deployed, and functionally proven — but it's prepared architecture for when policy enforcement is actually wired into a live decision path, not a fix that's currently defending an exploited call site.
...
- **L5 is deployed and functionally proven, but not yet exercised by a production decision path.** `glc-policy-engine` runs the real decision in its own Modal Function; verified live that a local monkey-patch of `glc.policy.engine.evaluate` has zero effect on it. But nothing in `glc/routes/*.py` calls the policy engine for a real decision yet — the agent runtime is still a stub (matching `glc.policy.engine.evaluate` itself, currently only referenced from test files) — so this closes an architectural gap ahead of when it's needed rather than a currently-exploited call site. When the agent runtime graduates from stub, it must call `glc.policy.remote.evaluate_remote()` (gated by `is_remote()`), not the local `glc.policy.engine.evaluate`, or this fix does nothing.
